...
const DEFAULT_ROOM="112233";
async function seedDefaultRoom(){
  try{
    if(roomsCol){
      await roomsCol.updateOne(
        {roomName:DEFAULT_ROOM},
        {$setOnInsert:{roomName:DEFAULT_ROOM,createdAt:Date.now()},$set:{updatedAt:Date.now()}},
        {upsert:true}
      );
    }
    liveRoom(DEFAULT_ROOM); // ensures it exists even without MongoDB configured
  }catch(e){console.error("seedDefaultRoom",e)}
}
...
io.on("connection",s=>{
  ...
  s.on("editMessage",async payload=>{
    const n=s.data.roomName,u=s.data.userName;if(!n||!u)return;
    const id=String(payload?.id||"").slice(0,120);
    const text=String(payload?.text||"").trim().slice(0,2000);
    if(!id||!text)return;
    try{
      if(col){
        const result=await col.updateOne({id,roomName:n,name:u},{$set:{text,edited:true}});
        if(!result.matchedCount)return;
      }
      io.to(n).emit("messageEdited",{id,text,edited:true});
  ...
  // Explicit "Leave" click: the member fully exits the room (removed from the list).
  s.on("leaveRoom",async()=>{
    await exitRoom(s);
  });

  // Tab close / refresh / dropped connection: keep the member in the room, just mark them offline.
  s.on("disconnect",async()=>{await markOffline(s)});
});

async function removeFromLiveRoom(s){
  const n=s.data.roomName;
  const r=liveRooms.get(n);
  if(r){
    r.delete(s.id);
    if(!r.size && n!==DEFAULT_ROOM)liveRooms.delete(n);
  }
}
...
async function exitRoom(s){
  const n=s.data.roomName,u=s.data.userName;
  if(!n||!u)return;
  try{
    await removeFromLiveRoom(s);
    if(membersCol)await membersCol.deleteOne({roomName:n,name:u});
    if(pushCol)await pushCol.deleteMany({roomName:n,userName:u});
    await emitMembers(n);
  }catch(e){console.error(e)}
  delete s.data.roomName;delete s.data.userName;
}

db().then(seedDefaultRoom).then(()=>server.listen(PORT,()=>console.log("FiveChat v8 on "+PORT))).catch(e=>{console.error(e);process.exit(1)})
