...
## v43 UI and group-management updates
- Login hides the PIN label; signup shows **Create 4 Digit PIN**.
- Group three-dot menu now includes Group info, Edit group name, Exit group, and Delete group for admins.
- Every group member can edit the group name on desktop and mobile.
- Group members can be promoted/demoted as admins; admins can remove members; admins can also exit. If the last admin exits, another remaining member is promoted automatically.
- Message text selection/copy/drag is disabled; Edit/Unsend actions remain text-only without icons.
- Added smooth member-management and group-menu transitions.


## v43 updates
- Login PIN has no visible PIN label; signup shows **Create 4 Digit PIN**.
- Group three-dot actions work on desktop and mobile, including Group info, Edit group name, Exit group, and admin-only Delete group.
- All group members can edit the group name; the desktop/mobile group-info pencil is enabled.
- Group admins can click a member to get animated Make admin/Remove admin/Remove member actions.
- Message text selection/copying is disabled and message Edit/Unsend menu entries have no icons.
- Unread counts are reconstructed from MongoDB after login, so messages received while offline/logged out appear beside the correct recipient/group instead of only after a live session.
- The global unread count beside ONLINE MEMBERS was removed; unread badges remain on the relevant recipient/group rows only.
