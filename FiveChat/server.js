...
let db,accounts,messages,groups,pushSubscriptions,groupReads,groupMutes;
...
async function init(){
  ...
  groupMutes=db.collection('groupMutes');
  ...
  await groupMutes.createIndex({nameLower:1,groupId:1},{unique:true});
...
async function mutedGroupIds(name,groupsList){if(!groupMutes||!name||!groupsList?.length)return [];const ids=groupsList.map(g=>g.id);return (await groupMutes.find({nameLower:name.toLowerCase(),groupId:{$in:ids}},{projection:{_id:0,groupId:1}}).toArray()).map(x=>x.groupId)}
async function isGroupMuted(name,groupId){
  if(!name||!groupId||!groupMutes)return false;
  return !!(await groupMutes.findOne({nameLower:name.toLowerCase(),groupId}));
}
async function setGroupMute(name,groupId,muted){
  if(!name||!groupId||!groupMutes)return;
  const filter={nameLower:name.toLowerCase(),groupId};
  if(muted) await groupMutes.updateOne(filter,{$set:{nameLower:name.toLowerCase(),groupId,updatedAt:Date.now()}},{upsert:true});
  else await groupMutes.deleteOne(filter);
}
async function sendPushToUser(name,payload){
  if(!pushEnabled||!pushSubscriptions||!name)return;
  if(payload.kind==='group' && await isGroupMuted(name,payload.groupId))return;
...
io.on('connection',s=>{
 ...
 s.on('login',async p=>{
  ...
  try{
    ...
    const mutedGroups=await mutedGroupIds(a.name,groupsNow);
    s.emit('loggedIn',{user:{name:a.name,createdAt:a.createdAt},people:peopleNow,groups:groupsNow,unread:unreadNow,mutedGroups});
    await emitPeople();
  }catch(e){
 ...
 s.on('pushSubscribe',async p=>{if(!s.data.userName||!p?.subscription?.endpoint)return;try{await savePushSubscription(s.data.userName,p.subscription)}catch(e){console.error('Push subscription save failed:',e.message)}});
 s.on('pushUnsubscribe',async p=>{if(!s.data.userName||!p?.endpoint)return;try{await removePushSubscription(s.data.userName,p.endpoint)}catch(e){}});
 s.on('pushActivity',async p=>{if(!s.data.userName||!p?.endpoint||!pushSubscriptions)return;await pushSubscriptions.updateOne({endpoint:p.endpoint,nameLower:s.data.userName.toLowerCase()},{$set:{visible:!!p.visible,activeChat:p.activeChat||null,lastActiveAt:Date.now(),updatedAt:Date.now()}})});
 s.on('setGroupMute',async(p,ack)=>{const id=String(p?.groupId||''),me=s.data.userName,muted=!!p?.muted;const g=await getGroup(id);if(!g||!me||!g.membersLower.includes(me.toLowerCase())){ack?.({ok:false,error:'You are not a member of this group.'});return}await setGroupMute(me,id,muted);ack?.({ok:true,groupId:id,muted});});
 ...
 s.on('addGroupMembers',async(p,ack)=>{const id=String(p?.groupId||''),me=s.data.userName,list=Array.isArray(p?.members)?p.members.map(x=>String(x).trim().slice(0,30)).filter(Boolean):[];if(!id||!me||!list.length){ack?.({ok:false,error:'Select at least one person.'});return}const g=await getGroup(id);if(!g||!g.membersLower.includes(me.toLowerCase())){const error='Only a group member can add people.';s.emit('groupMemberError',error);ack?.({ok:false,error});return}const existing=new Set(g.membersLower);const candidates=await Promise.all(list.map(n=>findAccount(n)));const added=[];for(const a of candidates){if(a&&!existing.has(a.nameLower)){existing.add(a.nameLower);g.members.push(a.name);g.membersLower.push(a.nameLower);added.push(a.name)}}if(!added.length){const error='No new people were selected.';s.emit('groupMemberError',error);ack?.({ok:false,error});return}g.updatedAt=Date.now();if(groups)await groups.updateOne({id},{$set:{members:g.members,membersLower:g.membersLower,updatedAt:g.updatedAt}});else mem.groups.set(id,g);const event=await saveGroupEvent(id,`${me} added ${added.join(', ')} to the group`);ack?.({ok:true,group:g,added});for(const sock of io.sockets.sockets.values())if(g.membersLower.includes(sock.data.userName?.toLowerCase())){sock.emit('groupUpdated',g);sock.emit('groupEvent',event)}});
