...
const liveRooms=new Map(),roomPasswords=new Map([["1122","123456"]]);
...
io.on("connection",s=>{
  ...
  s.on("joinRoom",async(payload)=>{
    try{
      ...
      const password=String(payload?.password||"").trim().slice(0,80);
      const exists=await roomExists(roomName);
      if(!password)return s.emit("joinError","Enter the room password.");
      if(mode==="create" && exists)return s.emit("joinError","That room ID already exists. Choose another ID.");
      if(mode==="existing" && !exists)return s.emit("joinError","Room not found. Check the room ID.");

      if(!exists){
        roomPasswords.set(roomName,password);
        if(roomsCol)await roomsCol.insertOne({roomName,password,createdAt:Date.now(),updatedAt:Date.now()});
      }else{
        let saved=roomPasswords.get(roomName);
        if(roomsCol){
          const record=await roomsCol.findOne({roomName},{projection:{password:1}});
          if(record?.password)saved=record.password;
          else if(record&&!record.password){saved=password;await roomsCol.updateOne({roomName},{$set:{password,updatedAt:Date.now()}});}
        }
        if(saved!==password)return s.emit("joinError","Incorrect room password.");
        if(roomsCol)await roomsCol.updateOne({roomName},{$set:{updatedAt:Date.now()}});
      }
  ...
  s.on("editMessage",async payload=>{
    const n=s.data.roomName,u=s.data.userName;
    const id=String(payload?.id||"").slice(0,120);
    const text=String(payload?.text||"").trim().slice(0,2000);
    if(!n||!u||!id||!text)return;
    try{
      const updated={id,text,time:Date.now()};
      if(col){
        const result=await col.updateOne({id,roomName:n,name:u},{$set:{text,time:updated.time}});
        if(!result.matchedCount)return s.emit("sendError","Unable to edit that message.");
      }
      io.to(n).emit("messageEdited",updated);
    }catch(e){console.error(e);s.emit("sendError","Unable to edit message. Try again.")}
  });
