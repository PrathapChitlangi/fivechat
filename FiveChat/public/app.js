...
async function setupPushNotifications(){if(!me||!('serviceWorker' in navigator)||!('PushManager' in window)||!window.isSecureContext)return false;try{const reg=await navigator.serviceWorker.register('/sw.js');if(Notification.permission!=='granted'){if(Notification.permission==='default'){const permission=await Notification.requestPermission();if(permission!=='granted')return false}else return false}let sub=await reg.pushManager.getSubscription();if(!sub){const key=await fetch('/api/push/public-key').then(r=>r.ok?r.text():'');if(!key)return false;sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:urlBase64ToUint8Array(key)})}pushSubscription=sub;s.emit('pushSubscribe',{subscription:sub.toJSON()});sendPushActivity();return true}catch(e){console.warn('Push setup unavailable',e);return false}}
...
async function updateNotificationSetting(){if(!('Notification' in window)||!('serviceWorker' in navigator)||!('PushManager' in window)){showToast('Browser notifications are not supported here');return}const next=!notificationEnabled;notificationEnabled=next;localStorage.setItem(PUSH_PREF_KEY,String(next));updateNotificationLabel(next,next);if(s.connected)s.emit('pushPreference',{enabled:next});if(!next){updateNotificationLabel(false,false);showToast('Notifications off');return}if(pushSubscription){updateNotificationLabel(true,false);showToast('Notifications on');return}const ok=await setupPushNotifications();if(ok){updateNotificationLabel(true,false);showToast('Notifications on')}else{notificationEnabled=false;localStorage.setItem(PUSH_PREF_KEY,'false');if(s.connected)s.emit('pushPreference',{enabled:false});updateNotificationLabel(false,false);showToast('Notifications could not be enabled')}}
...
s.on('loggedIn',d=>{
  ...
  me=d.user; people=d.people||[]; groups=d.groups||[]; saveSession(); showApp(); renderPeople(); renderGroups(); welcome(); updateNotificationLabel(notificationEnabled,false);
...
function renderGroups(){
  ...
  groups.forEach(g=>{
    ...
    li.querySelector('.groupMore').onclick=e=>{e.preventDefault();e.stopPropagation();showGroupMenu(g,e.currentTarget)};
    $('groupList').append(li);
  });
}
...
function openMemberActions(g,name,anchor){
  ...
  if(name.toLowerCase()===me.name.toLowerCase()) menu.innerHTML='<div class="memberActionHint">This is you</div>';
  else if(!isGroupAdmin(g)) menu.innerHTML='<div class="memberActionHint">Member · no management actions</div>';
  else {
    let buttons='';
...
s.on('groupMessage',m=>{
  ...
  if(active){addMessage(m);scroll();if(!isChatVisible())bumpUnread('group',m.groupId)}
...
window.addEventListener('load',()=>{if('serviceWorker' in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{});updateNotificationLabel(notificationEnabled,false)});
