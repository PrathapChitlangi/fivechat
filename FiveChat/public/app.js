...
function closeMoreMenus(){
  document.querySelectorAll('.floatingGroupMenu').forEach(x=>x.remove());
  document.querySelectorAll('.groupMore[aria-expanded="true"],.chatAvatar[aria-expanded="true"]').forEach(x=>x.setAttribute('aria-expanded','false'));
}
...
$('chatAvatar').onclick=e=>{
  ...
  const g=getGroup(selected.id); if(!g)return;
  const existing=document.querySelector('.groupMoreMenu');
  if(existing){closeMoreMenus();return;}
  showGroupMenu(g,e.currentTarget);
};
$('emojiBtn').onclick=()=>{if($('message').disabled)return;$('message').setRangeText('😊',$('message').selectionStart,$('message').selectionEnd,'end');$('message').focus();resizeComposer();announceTyping()};
...
function updateNotificationLabel(on){const btn=$('profileNotifications'),label=$('notificationLabel'),state=$('notificationState');if(label)label.textContent=`Notifications: ${on?'On':'Off'}`;if(state)state.textContent=on?'Push alerts enabled':'Tap to enable';if(btn){btn.classList.toggle('isOn',!!on);btn.setAttribute('aria-pressed',on?'true':'false')}}
