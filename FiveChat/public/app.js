...
function ensureAccountCenter(){
 const card=document.querySelector('#settingsModal .settingsCard');if(!card)return;
 let sec=document.getElementById('accountCenter');if(!sec){sec=document.createElement('div');sec.id='accountCenter';sec.className='settingsSection accountCenter';card.appendChild(sec)}
 sec.innerHTML=`<b>Account & privacy</b>
 ...
 <div class="privacyExtra"><label>Profile picture URL<input id="profileAvatarUrl" placeholder="https://…"></label><label class="settingCheck"><input id="privacyPhoto" type="checkbox"> Show profile picture</label><label class="settingCheck"><input id="notifyMessages" type="checkbox"> Message notifications</label><label class="settingCheck"><input id="notifyRequests" type="checkbox"> Request notifications</label><label class="settingCheck"><input id="notifyGroups" type="checkbox"> Group notifications</label><label class="settingCheck"><input id="notifyMentions" type="checkbox"> Mention notifications</label><label class="settingCheck"><input id="notifyReplies" type="checkbox"> Reply notifications</label><label>Accent color<input id="accentColor" type="color" value="#2563eb"></label><label>Message bubbles<select id="bubbleStyle"><option value="default">Default</option><option value="compact">Compact</option><option value="soft">Soft</option></select></label></div><div id="accountCenterResults" class="accountCenterResults"></div>`;
 ...
 document.getElementById('deleteAccountBtn').onclick=()=>{const pin=prompt('Enter your current 4-digit PIN to permanently delete your account');if(pin&&confirm('This permanently deletes your account and retained account data. Continue?'))s.emit('deleteAccount',{pin},r=>{if(r?.ok){sessionStorage.removeItem(SESSION);sessionStorage.removeItem(ACTIVITY_KEY);location.reload()}else showToast(r?.error||'Could not delete account')})};
}
function openSettings(){s.emit('getSettings',{},r=>{currentSettings=r?.settings||{privacy:{online:true,lastSeen:true,readReceipts:true,requests:'everyone'},compact:false};s.emit('getProfile',{name:me.name},p=>{$('profileBio').value=p?.profile?.bio||'';if(document.getElementById('profileAvatarUrl'))document.getElementById('profileAvatarUrl').value=p?.profile?.avatar||''});$('privacyOnline').checked=currentSettings.privacy?.online!==false;$('privacyLastSeen').checked=currentSettings.privacy?.lastSeen!==false;$('privacyRead').checked=currentSettings.privacy?.readReceipts!==false;$('requestPrivacy').value=currentSettings.privacy?.requests||'everyone';ensureAccountCenter();applyTheme();$('settingsModal').classList.remove('hidden');requestAnimationFrame(()=>$('settingsModal').classList.add('modalVisible'))})}
