...
s.on('loggedIn',d=>{
  ...
  me=d.user; people=d.people||[]; groups=d.groups||[]; Object.keys(unread).forEach(k=>delete unread[k]); Object.entries(d.unread||{}).forEach(([k,v])=>{if(Number(v)>0)unread[k]=Number(v)}); saveUnread(); saveSession(); showApp(); renderPeople(); renderGroups(); welcome();
...
function showGroupMenu(g,anchor){
  ...
  menu.innerHTML=`<button type="button" data-info><span>☰</span> Group info</button><button type="button" data-edit><span>✎</span> Edit group name</button><button type="button" data-leave class="dangerOption"><span>↪</span> Exit group</button>${isAdmin?'<button type="button" data-delete class="dangerOption"><span>⌫</span> Delete group</button>':''}`;
  document.body.append(menu);
  const r=anchor.getBoundingClientRect(), mw=190, mh=isAdmin?164:124;
...
function renderGroupInfo(g){
  ...
  const memberRows=g.members.map(name=>{
    ...
    const controls=isAdmin&&name.toLowerCase()!==me.name.toLowerCase()?`<button class="memberActionBtn" data-member-action="${escapeHtml(name)}" type="button" aria-label="Manage ${escapeHtml(name)}">⋮</button>`:'';
  ...
  $('groupInfoPanel').innerHTML=`<div class="infoHeader"><div><span class="overline">GROUP INFO</span><div class="groupInfoTitleRow"><h3>${escapeHtml(g.name)}</h3><button class="groupEditPencil" type="button" aria-label="Edit group name" title="Edit group name"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 17.25V20h2.75L17.81 8.94l-2.75-2.75-2.75-2.75L4 17.25Zm15.71-9.04c.38-.38.38-1-0-1.38l-2.54-2.54a.975.975 0 0 0-1.38 0l-1.36 1.36 2.75 2.75 1.53-1.19Z"/></svg></button></div><small>${g.members.length} members · ${admins.length} admin${admins.length===1?'':'s'}</small></div><button class="infoClose" type="button">×</button></div><div class="infoMembers">${memberRows}</div><div class="groupInfoActions">${isAdmin?'<button class="addMemberBtn" type="button">＋ Add people</button>':''}<button class="leaveGroupBtn" type="button">↪ Exit group</button></div>`;
  ...
  $('groupInfoPanel').querySelectorAll('[data-member-action]').forEach(btn=>btn.onclick=e=>{e.stopPropagation();openMemberActions(g,btn.dataset.memberAction)});
...
function openMemberActions(g,name){
  ...
  document.body.append(menu);
  const anchor=[...$('groupInfoPanel').querySelectorAll('[data-member-action]')].find(x=>x.dataset.memberAction===name);const r=anchor?.getBoundingClientRect()||{right:innerWidth/2,top:innerHeight/2,bottom:innerHeight/2};
  const mw=180,mh=108;let left=Math.min(r.right-mw,innerWidth-mw-10);left=Math.max(10,left);let top=r.bottom+7;if(top+mh>innerHeight-10)top=Math.max(10,r.top-mh-7);menu.style.left=left+'px';menu.style.top=top+'px';
