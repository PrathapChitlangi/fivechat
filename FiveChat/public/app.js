...
let nicknames = {}, authMode = 'login', typingTimer = null, typingActive = false, typingLastSent = 0, pendingFiles = [], editingId = null, logoutInProgress = false, authRequestId = 0, longPressTimer = null;
...
loadUnread();
function saveSession(){ if(me) sessionStorage.setItem(SESSION, JSON.stringify({name:me.name,token:window.__sessionToken||''})); sessionStorage.setItem(ACTIVITY_KEY,String(Date.now())); }
...
function renderRequestBadge(){const el=$('requestCount');if(!el)return;const n=requestCount();el.textContent=n>99?'99+':String(n);el.classList.toggle('hidden',n===0);const summary=$('requestsPanelSummary');if(summary)summary.textContent=n?`${n} pending request${n===1?'':'s'}`:'No pending requests'}
function refreshRequestUI(){renderRequestBadge();renderPeople();renderRequestsPanel()}
function renderRequestsPanel(){const body=$('requestsPanelBody'),toggle=$('requestToggle');if(!body||!toggle)return;const inc=chatRequests.incoming||[],out=chatRequests.outgoing||[];const open=toggle.getAttribute('aria-expanded')==='true';let html='';if(inc.length)html+=`<div class="requestPanelLabel">NEW INCOMING</div>`+inc.map(r=>`<div class="requestPanelRow"><span class="avatar requestPanelAvatar" style="--avatarHue:${senderHue(r.from)}">${escapeHtml(r.from[0].toUpperCase())}</span><div class="requestPanelText"><b>${escapeHtml(displayName(r.from))}</b><small>Wants to connect with you</small></div><div class="requestPanelActions"><button type="button" data-accept-request="${escapeHtml(r.id)}">Accept</button><button type="button" class="reject" data-decline-request="${escapeHtml(r.id)}">Decline</button></div></div>`).join('');if(out.length)html+=`<div class="requestPanelLabel">SENT / PENDING</div>`+out.map(r=>`<div class="requestPanelRow"><span class="avatar requestPanelAvatar" style="--avatarHue:${senderHue(r.to)}">${escapeHtml(r.to[0].toUpperCase())}</span><div class="requestPanelText"><b>${escapeHtml(displayName(r.to))}</b><small>Waiting for acceptance</small></div><button type="button" class="requestPanelCancel" data-cancel-request="${escapeHtml(r.id)}">Cancel</button></div>`).join('');if(!html)html='<div class="requestPanelEmpty"><span>♡</span><b>No new requests</b><small>New connection requests will appear here.</small></div>';body.innerHTML=html;body.classList.toggle('isOpen',open);toggle.setAttribute('aria-expanded',open?'true':'false');toggle.setAttribute('aria-label',open?'Hide new requests':'Show new requests')}
...
function auth(){
  ...
  logoutInProgress=false; window.__loginPin=pin; const request=++authRequestId; $('authBtn').disabled=true; $('authBtn').classList.add('authLoading'); $('authBtn').innerHTML='<span class="loginSpinner"></span> Connecting…';
...
s.on('authError',m=>{$('authBtn').disabled=false;$('authBtn').classList.remove('authLoading');updateAuth();$('authError').textContent=m;$('authCard').classList.remove('shake');void $('authCard').offsetWidth;$('authCard').classList.add('shake')});
s.on('loggedIn',d=>{
  if(logoutInProgress)return;
  $('authBtn').disabled=false;$('authBtn').classList.remove('authLoading');
  window.__sessionToken=d.sessionToken||window.__sessionToken||'';
  me=d.user; people=d.people||[]; groups=d.groups||[]; connections=(d.connections||[]).map(x=>String(x).toLowerCase()); chatRequests=d.requests||{incoming:[],outgoing:[]}; applyUnreadFromServer(d.unread); saveSession(); showApp(); s.emit('getNicknames',{},r=>{nicknames=r?.nicknames||{};renderPeople();if(selected.type==='direct'){const p=people.find(x=>x.name===selected.name);if(p){$('chatName').textContent=displayName(p.name);}}}); s.emit('getSettings',{},r=>{currentSettings=r?.settings||currentSettings;applyTheme()}); renderRequestBadge(); renderPeople(); renderGroups(); renderRequestsPanel(); const inviteToken=new URLSearchParams(location.search).get('groupInvite'); if(inviteToken){s.emit('joinGroupInvite',{token:inviteToken},r=>{if(r?.pending)showToast('Join request sent to group admins');else if(r?.ok)showToast(r.already?'Already in group':'Joined group');history.replaceState({},'',location.pathname)})} welcome(); startInactivityWatch(); updateNotificationLabel(notificationEnabled,false);
  s.emit('presence',{visible:true}); startHeartbeat(); if(notificationEnabled)setupPushNotifications();
});

function displayName(name){const n=nicknames[String(name||'').toLowerCase()];return n||name}
function renderPeople(){
  if(!me)return;
  const q=$('search').value.toLowerCase();
  const connectedSet=new Set(connections.map(x=>String(x).toLowerCase()));
  const arr=people.filter(p=>p.nameLower!==me.name.toLowerCase()&&connectedSet.has(p.nameLower)&&p.name.toLowerCase().includes(q)).sort((a,b)=>Number(b.online)-Number(a.online)||a.name.localeCompare(b.name));
  $('peopleList').innerHTML='';
  if(!arr.length){const li=document.createElement('li');li.className='peopleEmpty';li.innerHTML='<span>◎</span><b>No connected members</b><small>Accepted connections will appear here.</small>';$('peopleList').append(li)}
  arr.forEach(p=>{
    const li=document.createElement('li'); li.className='person '+(selected.type==='direct'&&selected.name===p.name?'selected':'');
    const count=unreadFor('direct',p.name); const badge=count>99?'99+':count;
    const status=p.online?'Online':`Offline · ${timeSince(p.lastSeen)}`;
    li.innerHTML=`<span class="avatar personAvatar" style="--avatarHue:${senderHue(p.name)}${p.avatar?`;background-image:url('${escapeHtml(p.avatar)}');background-size:cover;background-position:center;color:transparent`:''}">${escapeHtml(p.name[0].toUpperCase())}</span><span class="personText"><b>${escapeHtml(displayName(p.name))}</b><small class="statusLine ${p.online?'onlineStatus':'offlineStatus'}" title="${escapeHtml(p.online?'Online now':lastSeenTitle(p.lastSeen))}"><i class="statusIndicator ${p.online?'isOnline':'isOffline'}"></i>${status}</small></span>${count?`<b class="unread" data-unread-key="${escapeHtml(unreadKey('direct',p.name))}" aria-label="${count} unread messages">${badge}</b>`:''}<button class="personChatAction" type="button">Chat</button>`;
    li.onclick=e=>{if(!e.target.closest('.personChatAction'))openDirect(p)};
    li.querySelector('.personChatAction')?.addEventListener('click',e=>{e.stopPropagation();openDirect(p)});
...
function buildAddMembers(g){const existing=new Set(g.membersLower||g.members.map(x=>x.toLowerCase()));$('addMembersList').innerHTML='';people.filter(p=>p.nameLower!==me.name.toLowerCase()&&!existing.has(p.nameLower)).forEach(p=>{const l=document.createElement('label');l.className='memberChoice';l.innerHTML=`<input type="checkbox" value="${escapeHtml(p.name)}"><span class="choiceAvatar" style="--choiceHue:${senderHue(p.name)}">${escapeHtml(p.name[0].toUpperCase())}</span><span class="choiceText"><b>${escapeHtml(displayName(p.name))}</b><small>${p.online?'Online':'Offline · '+timeSince(p.lastSeen)}</small></span><span class="choiceCheck">✓</span>`;$('addMembersList').append(l)});if(!$('addMembersList').children.length)$('addMembersList').innerHTML='<div class="noPeopleToAdd">Everyone is already in this group.</div>'}
...
function showDirectInfo(p,anchor){
  ...
  const status=p.online?'Online':`Offline · ${timeSince(p.lastSeen)}`;
  menu.innerHTML=`<div class="directInfoHead"><span class="avatar directInfoAvatar" style="--avatarHue:${senderHue(p.name)}">${escapeHtml(p.name[0].toUpperCase())}</span><div><b>${escapeHtml(displayName(p.name))}</b><small class="${p.online?'isOnlineText':'isOfflineText'}">${status}</small></div><button type="button" class="directInfoClose" aria-label="Close">×</button></div><div class="directInfoMeta">${p.online?'Available now':`Last seen ${lastSeenTitle(p.lastSeen)}`}</div><button type="button" class="nicknameDirectBtn">✎ Set nickname</button><button type="button" class="clearDirectBtn">⌫ Clear chat history</button><button type="button" class="blockDirectBtn">🚫 Block user</button>`;
  document.body.append(menu);
  ...
  menu.querySelector('.nicknameDirectBtn').onclick=e=>{e.stopPropagation();const current=nicknames[p.name.toLowerCase()]||'';const value=prompt(`Set a nickname for ${p.name}`,current);if(value===null)return;s.emit('setNickname',{target:p.name,nickname:value},r=>{if(r?.ok){if(r.nickname)nicknames[p.name.toLowerCase()]=r.nickname;else delete nicknames[p.name.toLowerCase()];$('chatName').textContent=displayName(p.name);renderPeople();showToast(r.nickname?'Nickname updated':'Nickname removed');closeDirectInfo()}else showToast(r?.error||'Could not update nickname')})};menu.querySelector('.clearDirectBtn').onclick=e=>{e.stopPropagation();closeDirectInfo();openClearChatConfirm(p.name)};menu.querySelector('.blockDirectBtn').onclick=e=>{e.stopPropagation();closeDirectInfo();if(confirm(`Block ${p.name}? They will no longer be able to connect with you.`)){s.emit('blockUser',{name:p.name},result=>{if(result?.ok){connections=connections.filter(x=>x!==p.name.toLowerCase());showToast(`${p.name} blocked`);closeChat();refreshRequestUI()}else showToast(result?.error||'Could not block user')})}};
}
function openDirect(p){
  ...
  $('chatName').textContent=displayName(p.name);$('chatAvatar').textContent=p.name[0].toUpperCase();$('chatAvatar').disabled=false;$('chatAvatar').dataset.userName=p.name;setDirectStatus(p);
...
function applyTheme(){document.documentElement.dataset.theme='system';document.body.classList.toggle('compactMode',!!currentSettings?.compact);document.body.dataset.bubble=currentSettings?.bubble||'default';if(currentSettings?.accent)document.documentElement.style.setProperty('--accent',currentSettings.accent)}
...
function buildGroupMembers(){ $('groupMembers').innerHTML='';people.filter(p=>p.name!==me.name).forEach(p=>{const l=document.createElement('label');l.className='memberChoice';l.innerHTML=`<input type="checkbox" value="${escapeHtml(p.name)}"><span class="choiceAvatar" style="--choiceHue:${senderHue(p.name)}">${escapeHtml(p.name[0].toUpperCase())}</span><span class="choiceText"><b>${escapeHtml(displayName(p.name))}</b><small>${p.online?'Online':'Offline · '+timeSince(p.lastSeen)}</small></span><span class="choiceCheck">✓</span>`;$('groupMembers').append(l)}) }
...
s.on('chatRequests',d=>{chatRequests=d||{incoming:[],outgoing:[]};refreshRequestUI()});
...
function announceActive(){if(!me)return;if(document.visibilityState!=='visible'||!document.hasFocus())return;if(!s.connected){s.connect();return}s.emit('presence',{visible:true});$('myStatus').textContent='Online';$('profileStatus').textContent='Online';const p=people.find(x=>x.name===selected.name);if(selected.type==='direct'&&p)setDirectStatus({...p,online:true,lastSeen:Date.now()});if(selected.type==='direct'&&selected.name)s.emit('markSeen',{with:selected.name});if(selected.type==='group'&&selected.id)s.emit('groupSeen',{groupId:selected.id});touchActivity()}
...
window.addEventListener('focus',announceActive);window.addEventListener('pageshow',announceActive);window.addEventListener('blur',()=>{if(me&&s.connected)s.emit('presence',{visible:false})});window.addEventListener('pagehide',()=>{if(me&&s.connected)s.emit('presence',{visible:false})});
...
function performLogout(){if(pushSubscription&&s.connected)sendPushActivity();sessionStorage.removeItem(SESSION);sessionStorage.removeItem(ACTIVITY_KEY);window.__sessionToken='';window.__loginPin='';
