...
function showGroupMenu(g,anchor){
  ...
  menu.innerHTML=g.createdBy===me.name?'<button type="button" data-delete class="dangerOption">Delete group</button>':'<div class="menuHint">Group options</div>';
  anchor.closest('.groupRow')?.append(menu);
  const del=menu.querySelector('[data-delete]');
  if(del)del.onclick=e=>{e.stopPropagation();closeMoreMenus();groups=groups.filter(x=>x.id!==g.id);if(selected.type==='group'&&selected.id===g.id)closeChat();renderGroups();s.emit('deleteGroup',{id:g.id})};
}
function closeMoreMenus(){document.querySelectorAll('.floatingGroupMenu').forEach(x=>x.remove())}

document.addEventListener('click',e=>{if(!e.target.closest('.groupMore'))closeMoreMenus();if(!e.target.closest('.meProfile'))$('profileMenu').classList.add('hidden');if(!e.target.closest('#groupInfoPanel')&&!e.target.closest('#chatAvatar'))closeGroupInfo();if(!e.target.closest('#contextMenu')&&!e.target.closest('.msg'))hideContext()});
...
function openDirect(p){
  hideContext();stopAllTyping(); selected={type:'direct',name:p.name}; closeGroupInfo(); clearUnread('direct',p.name); $('chatHead').classList.remove('homeHidden');
...
function openGroup(g){
  hideContext();stopAllTyping(); selected={type:'group',id:g.id,name:g.name}; clearUnread('group',g.id); closeGroupInfo(); $('chatHead').classList.remove('homeHidden');
...
function addMessage(m){
  ...
  if(m.system){
    const d=document.createElement('div');d.className='systemMessage';d.dataset.id=m.id;d.innerHTML=`<span class="systemDot">✦</span><span>${escapeHtml(m.text)}</span>`;$('messages').append(d);return;
  }
  ...
  if(m.unsent){d.innerHTML=`${sender}<div class="messageContent"><div class="bubble unsent">Message unsent</div></div><small>${new Date(m.time).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</small>`;d.classList.remove('mine','theirs');d.classList.add('theirs');d.style.cursor='default';$('messages').append(d);return}
...
function positionContext(x,y){const menu=$('contextMenu');const w=145,h=84;menu.style.left=Math.max(8,Math.min(x,innerWidth-w-8))+'px';menu.style.top=Math.max(8,Math.min(y,innerHeight-h-8))+'px';menu.classList.remove('hidden')}
...
function attachMessageInteractions(el,m){
  ...
  el.addEventListener('dblclick',e=>{if(!touchActive&&innerWidth>760)showContext(e,m);touchActive=false});
  ...
  el.addEventListener('touchend',()=>{clearTimeout(longPressTimer);setTimeout(()=>touchActive=false,80)},{passive:true});
...
function resizeComposer(){const el=$('message');if(!el)return;const min=innerWidth<=390?44:innerWidth<=760?48:54;const fieldMin=innerWidth<=390?46:innerWidth<=760?50:56;el.style.height='auto';const next=Math.min(Math.max(el.scrollHeight,min),116);el.style.height=next+'px';$('messageField').style.height=Math.max(fieldMin,Math.min(next+2,122))+'px'} function scroll(){$('messages').scrollTop=$('messages').scrollHeight}
...
$('message').onfocus=()=>{if(selected.type)announceTyping()};
$('message').onblur=()=>{setTimeout(()=>{if(document.activeElement!==$('message'))stopTypingForCurrent()},0)};
...
function renderGroupInfo(g){
  ...
  $('groupInfoPanel').innerHTML=`<div class="infoHeader"><div><span class="overline">GROUP INFO</span><div class="groupInfoTitleRow"><h3>${escapeHtml(g.name)}</h3>${admin?'<button class="groupEditPencil" type="button" aria-label="Edit group name" title="Edit group name">✎</button>':''}</div><small>${g.members.length} members</small></div><button class="infoClose" type="button">×</button></div><div class="infoMembers">${memberRows}</div><div class="groupInfoActions">${admin?'<button class="addMemberBtn" type="button">＋ Add people</button>':'<div class="memberNote">Only the group admin can add or remove people.</div><button class="leaveGroupBtn" type="button">Leave group</button>'}</div>`;
  ...
  const edit=$('groupInfoPanel').querySelector('.groupEditPencil');if(edit)edit.onclick=e=>{e.stopPropagation();const n=prompt('Enter a new group name',g.name);if(n&&n.trim()&&n.trim()!==g.name)s.emit('renameGroup',{id:g.id,name:n.trim()})};
  const leave=$('groupInfoPanel').querySelector('.leaveGroupBtn');if(leave)leave.onclick=e=>{e.stopPropagation();if(confirm('Leave this group?'))s.emit('leaveGroup',{groupId:g.id})};
...
s.on('messageUnsent',d=>{if(editingId===d.id){editingId=null;$('message').value='';resizeComposer()}refreshMessage(d.id,el=>{el.dataset.unsent='1';el.classList.remove('mine');el.classList.add('theirs','unsentMsg');el.style.cursor='default';el.querySelector('.messageContent').innerHTML='<div class="bubble unsent">Message unsent</div>';el.querySelectorAll('.messageTicks').forEach(x=>x.remove())});if(d.from!==me?.name&&d.kind==='direct'){if(selected.type==='direct'&&selected.name===d.from)addSystemNotice('A message was unsent');else{bumpUnread('direct',d.from);showToast(`${d.from} unsent a message`)}}});
...
function renderTyping(){
  const active=[...typingUsers.entries()].filter(([k,v])=>v&&v.groupId===selected.id&&v.until>Date.now()).sort((a,b)=>a[0].localeCompare(b[0]));
...
function performLogout(){
  ...
  localStorage.removeItem(SESSION); me=null; selected={type:null,id:null,name:null}; clearInterval(window.__hb); clearTimeout(typingTimer); pendingFiles=[];editingId=null;typingUsers.clear();hideContext();
