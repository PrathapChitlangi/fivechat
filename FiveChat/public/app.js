...
function renderGroupInfo(g){
  ...
  $('groupInfoPanel').innerHTML=`<div class="infoHeader"><div><span class="overline">GROUP INFO</span><h3>${escapeHtml(g.name)}</h3><small>${g.members.length} members</small></div><button class="infoClose" type="button">×</button></div><div class="infoMembers">${memberRows}</div><div class="groupInfoActions">${admin?'<button class="addMemberBtn" type="button">＋ Add people</button><button class="editGroupBtn" type="button">✎ Edit name</button>':'<div class="memberNote">Only the group admin can add or remove people.</div><button class="leaveGroupBtn" type="button">Leave group</button>'}</div>`;
  ...
  const edit=$('groupInfoPanel').querySelector('.editGroupBtn');if(edit)edit.onclick=e=>{e.stopPropagation();const n=prompt('Enter a new group name',g.name);if(n&&n.trim()&&n.trim()!==g.name)s.emit('renameGroup',{id:g.id,name:n.trim()})};
...
$('confirmAddMembers').onclick=()=>{if(selected.type!=='group')return;const names=[...$('addMembersList').querySelectorAll('input:checked')].map(x=>x.value);if(!names.length)return;const btn=$('confirmAddMembers');btn.disabled=true;btn.textContent='Adding…';s.emit('addGroupMembers',{groupId:selected.id,members:names});setTimeout(()=>{btn.disabled=false;btn.innerHTML='Add selected <span>→</span>';if(!$('memberModal').classList.contains('hidden'))$('memberModal').classList.add('hidden')},650)};
...
s.on('messageUnsent',d=>{if(editingId===d.id){editingId=null;$('message').value='';resizeComposer()}refreshMessage(d.id,el=>{el.dataset.unsent='1';el.classList.remove('mine');el.classList.add('theirs','unsentMsg');el.style.cursor='default';el.querySelector('.messageContent').innerHTML='<div class="bubble unsent">Message unsent</div>';el.querySelectorAll('.messageTicks').forEach(x=>x.remove())})});
...
setInterval(()=>{renderTyping()},250);setInterval(()=>{if(me){renderPeople();if(selected.type==='direct'){const p=people.find(x=>x.name===selected.name);if(p)setDirectStatus(p)}if(selected.type==='group'&&!$('groupInfoPanel').classList.contains('hidden')){const g=getGroup(selected.id);if(g)renderGroupInfo(g)}}},15000);
