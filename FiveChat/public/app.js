...
function openGroup(g){
  ...
  enableComposer(); $('messages').innerHTML='<div class="loading">Loading…</div>'; closePeople(); s.emit('openGroup',{id:g.id}); if(s.connected)s.emit('groupSeen',{groupId:g.id});
}
...
$('closeChat').onclick=closeChat;$('chatAvatar').onclick=e=>{e.stopPropagation();if(selected.type==='group')toggleGroupInfo();else if(selected.type==='direct')openReceiverActions()};$('emojiBtn').onclick=()=>{if($('message').disabled)return;$('message').setRangeText('😊',$('message').selectionStart,$('message').selectionEnd,'end');$('message').focus();resizeComposer();announceTyping()};
...
s.on('typing',p=>{if(selected.type==='direct'&&p.from===selected.name&&p.to===me.name){$('typing').innerHTML=`<span class="typingPerson"><span class="typingDots"><i></i><i></i><i></i></span><b>${escapeHtml(p.from)}</b> is typing…</span>`;clearTimeout(window.__directTyping);window.__directTyping=setTimeout(()=>{$('typing').innerHTML=''},1900)}});
